              ...
              {themes.map((theme) => (
                <button
                  ...
                  className={`w-full p-4 rounded-lg border-2 transition-all duration-200 ${
                    gameState.settings.theme === theme.id
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-white font-bold mb-3">
                  🎵 Volume des Effets Sonores: {gameState.settings.soundVolume}%
                </label>
                <input
                  type="range"
                  min="0"
                  max="100"
                  value={gameState.settings.soundVolume}
              ...
              <div>
                <label className="block text-white font-bold mb-3">
                  🎶 Volume de la Musique: {gameState.settings.musicVolume}%
                </label>
                <input
                  type="range"
                  min="0"
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/20 rounded-lg p-3 text-center">
                ...
                <div className="text-white/80 text-sm">{gameState.totalClicks.toLocaleString('fr-FR')}</div>
              </div>
              <div className="bg-white/20 rounded-lg p-3 text-center">
                <div className="text-white font-bold">Argent Gagné</div>
                <div className="text-white/80 text-sm">₹{gameState.totalMoneyEarned.toLocaleString('fr-FR')}</div>
              </div>
              <div className="bg-white/20 rounded-lg p-3 text-center">
                <div className="text-white font-bold">Tickets Grattés</div>
                <div className="text-white/80 text-sm">{gameState.totalTicketsScratched}</div>
              </div>
              <div className="bg-white/20 rounded-lg p-3 text-center">
                <div className="text-white font-bold">Victoires Casino</div>
                <div className="text-white/80 text-sm">{gameState.totalCasinoWins}</div>
              </div>
              <div className="bg-white/20 rounded-lg p-3 text-center">
              ...
              <div className="bg-white/20 rounded-lg p-3 text-center">
                ...
                <div className="text-white/80 text-sm">
                  {gameState.guillaumes.reduce((sum, g) => sum + g.owned, 0)}
                </div>
              </div>
            </div>
          </div>
