...
export default function Settings({ gameState, onUpdateSettings, onResetGame }: SettingsProps) {
  ...
  const safeBlackjackWins = gameState?.blackjackWins || 0;
  const safeRouletteWins = gameState?.rouletteWins || 0;
  const safeSlotWins = gameState?.slotWins || 0;
  const safeBiggestWin = gameState?.biggestWin || 0;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-white/20 rounded-lg p-3 text-center">
                <div className="text-white font-bold text-sm">Blackjack Gagnés</div>
                <div className="text-white/80 text-lg">{safeBlackjackWins}</div>
              </div>
              <div className="bg-white/20 rounded-lg p-3 text-center">
                <div className="text-white font-bold text-sm">Plus Gros Gain</div>
                <div className="text-white/80 text-lg">₹{safeBiggestWin.toLocaleString('fr-FR')}</div>
              </div>
            </div>
          </div>
